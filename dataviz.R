...
# Tasks and effort percentiles to process
# p1/p2 are saved only at the 0.05 percentile (as before); p3 is saved for each
tasklist <- sprintf('task%02d', 1:13)
...
for (effortpercentile in percentiles) {

taskfolder = paste('outputs/', task, '/solutions/', sep="")
...
for (dof in dofs) {
  ...
  if (effortpercentile==0.05) {ggsave(paste(taskfolder, p1_filename, sep=""), p1, height=8, width=5, dpi=900)}
  ...
  if (effortpercentile==0.05) {ggsave(paste(taskfolder, p2_filename, sep=""), p2, height=8, width=5, dpi=900)}
  ...
  if (dof=='8dof-all') {
    ...
    if (effortpercentile == 0.05) {
      ggsave(paste(taskfolder, p1_tasktitle_filename, sep=""), p1_tasktitle, height=8, width=5, dpi=900)
      ggsave(paste(taskfolder, p2_tasktitle_filename, sep=""), p2_tasktitle, height=8, width=5, dpi=900)
      ggsave(paste(taskfolder, p2_optimal_all_filename, sep=""), p2_optimal_all, height=8, width=5, dpi=900)
    }
  }
  ...
  if (dof=='8dof-loweffort') {
    ...
    ggsave(paste(taskfolder, p3_filename, sep=""), p3, height=8, width=5, dpi=900)
  }
  
  # Clean work space
  gdata::keep(tasklist, percentiles, task, effortpercentile, dofs, taskfolder, musclenames, thoracoscapular, glenohumeral, thoracohumeral, elbow,
