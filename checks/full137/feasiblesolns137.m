% feasiblesolns137.m - feasible solution sampling for the UNREDUCED 137-element model.
%
% Identical to ../../feasiblesolns.m except that the task is fixed rather than
% prompted, and inputs/outputs are read from and written to checks/full137/outputs.
% The constraint construction, reserve actuators, stability matrix, seeds and
% sample counts are unchanged, so results are directly comparable with the
% 42-element model.
%
% Inputs are produced by inversedynamics137.py (which requires OpenSim 4.4).
%
% Runtime: roughly 22 min for all four DOF models. The polytope is 143 to 159
% dimensional here versus 48 to 64 for the reduced model, so sampling is slower.
...
% Single representative task: 50 N downward exertion
task = 'task03';
outfolder = fullfile(scriptdir, 'outputs', task);  % Locate directory
resultsdir = fullfile(outfolder, 'solutions');
...
% Define reserve actuators
reserves = diag([1 1 1 1 1 1 ...  % Trunk
    10 10 10 ...  % SC (yzx)
    10 10 10 ...  % AC (yzx)
    1 1 1 ...  % GH (yzyy)
    1 1 ...    % ELx-PSy
